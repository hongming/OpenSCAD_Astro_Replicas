...
//核心立面支撑面
module wing_support(wing_support_hole,wing_surpport_fixed_rod) {
        rotate([90, 90, 0]) {
...
//支架和固定轴承
module holder_attachment(telescope_array_angle) {
    rotate([90, 0, 90]) {
        ...
        translate([0, 0, -40 + 10]) {
            ...
            rotate([180, 0, -1 * tail_for_telescope_angle + 180 - telescope_array_angle])
            dovetail_2047studio_dovetail_dule_saddle(195, 2);
        }
    }
}
 
//有电机的立板
module holder_with_motor_attachment(telescope_array_angle) {
    rotate([90, 0, 90]) {
        //用于支撑电机的圆板
...
module dovetail_2047studio_dovetail_dule_saddle(tail_for_telescope_length, position) {
    if (position == 1) {
        //旋转把手
...
module XRU8022_cross_roller_bearing() {
    color("silver")
    difference() {
        cylinder(h = 22, d = 165, center = true);
        ...
        difference() {
            cylinder(h = 24, d = 125, center = true);
            cylinder(h = 26, d = 120, center = true);
        }
    }
    color("black")
    difference() {
        cylinder(h = 16, d = 125, center = true);
